...
void encode_expr(EXPR expr)
{
  ...
  switch (expr->tag) {
    ...
    case FCALL:	       encode_fcall(expr->u.fcall_or_array_access.function_or_array, expr->u.fcall_or_array_access.args_or_indices);
      		       break;
    case ARRAY_ACCESS:
                       encode_arrayaccess(expr->u.fcall_or_array_access.function_or_array, expr->u.fcall_or_array_access.args_or_indices);
                       break;
...
void encode_arrayaccess(EXPR expr, EXPR_LIST indices) {
   ...
   align = simple_size(array_type);
   size = get_array_size(array_type, align);
